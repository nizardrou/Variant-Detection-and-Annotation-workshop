...
The output of BWA mem is a SAM alignment file, and there is little to no benefit in working with the SAM file, the BAM formats is better (less space on disk as well as the ability to index it for fast processing). The SAM to BAM conversion, and the BAM sorting will be performed using SAMTools.
